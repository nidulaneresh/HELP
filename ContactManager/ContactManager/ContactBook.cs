...
namespace ContactManager
{
    public class ContactBook
    {
        ...
        //DataTables used for DataGridViews
        //EmployeeTable includes Trainees
        ...
        private void DefineEmployeeTable()
        {
            ...
            EmployeeTable.Columns.Add("Max Trainee Years", typeof(int));         //24
            EmployeeTable.Columns.Add("Current Trainee Year", typeof(int));       //25
        }
        ...
        {
            ContactDocTable.Columns.Add("ID", typeof(int));                 //0
            ContactDocTable.Columns.Add("Kunde", typeof(string));           //1
            ContactDocTable.Columns.Add("Mitarbeiter", typeof(string));     //2
            ContactDocTable.Columns.Add("Datum", typeof(DateTime));         //3
            ContactDocTable.Columns.Add("Notizen", typeof(string));         //4
        }

        //Update ContactDocTable based on ContactDoc Dictionary
        public void FillContactDocTable()
        {
            ContactDocTable.Clear();

            foreach (KeyValuePair<int, ContactDoc> keypair in ContactDocs)
            {
                ContactDocTable.Rows.Add(keypair.Value.GetDataRow(ContactDocTable, this));
            }

            Disk.Save(this);
        }

        //Ensures that Trainees are stored as Trainees and not Employees
        public void ConvertEmployeesToTraineesJSON()
        {
            foreach (DataRow row in EmployeeTable.Rows)
            {
                object value = row.Field<object>(24);
                if (value != null)
                {
                    Trainee trainee;
                    trainee = new Trainee(Employees[row.Field<int>(0)]);
                    trainee.MaxTraineeYears = row.Field<int>(24);
                    trainee.CurrentTraineeYear = row.Field<int>(25);
                    Employees[row.Field<int>(0)] = trainee;
                }
            }
        }
    }
